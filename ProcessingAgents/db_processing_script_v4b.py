...
AND ADDED POLICE_STATIONS AND DISTRICT COLUMNS WHERE IT WAS REQUIRED (PROCESSED_DATA, FIR_CASES) FOR USER ROLES
Latest file before this file for processing is db_processing_script_v4a.py
'''
        ...
        queries = {
            ...
            'conference_calls': """
                    SELECT 
                        l.district_id,
                        l.police_station,
                        HOUR(FROM_UNIXTIME(o.call_time_unix)) AS hour,
                        COUNT(*) AS count
                    FROM `outgoing_calls_logs_preprocessed` o
                    JOIN `15_preprocessed` l 
                        ON o.lead_id = l.lead_id
                    WHERE 
                        l.district_id IS NOT NULL 
                        AND o.tx_type='ATT' 
                        AND o.call_time_unix BETWEEN %s AND %s
                    GROUP BY l.district_id, l.police_station, hour;
        ...
        for key, query in queries.items():
            ...
            if key == 'counts':
                ...
                for district_id,police_station, hour, count in cursor.fetchall():
                    ...
                    if (district_id,police_station, hour) not in results:
                        results[(district_id,police_station, hour)] = {}
                    results[(district_id,police_station, hour)]['conference_calls'] = count
    ...
    try:
        ...
        for (district_id,police_station, hour), result in results.items():
            ...
            cursor.execute(insert_query, (
                row['date'], row['hour'], row['district_id'],row['police_station'], row['total_calls'], row['generated_cases'],row['siraiki'],
...
if __name__ == '__main__':
    start_date = datetime.strptime('25-11-24', '%d-%m-%y')
    end_date = datetime.strptime('30-11-24', '%d-%m-%y')
