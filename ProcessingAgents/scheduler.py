    ...
    try:
        ...
        for (district_id, police_station, hour), result in results.items():
            ...
            cursor.execute(insert_query, (
                row['date'], row['hour'], row['district_id'], row['police_station'], row['total_calls'],
                row['generated_cases'], row['siraiki'],
                row['punjabi'], row['potohari'], row['vwps'], row['traffic'], row['english'], row['app_alerts'],
                row['transfered'],
...
if __name__ == '__main__':
    # start_date = datetime.strptime('06-12-24', '%d-%m-%y')
    # end_date = datetime.strptime('06-12-24', '%d-%m-%y')
