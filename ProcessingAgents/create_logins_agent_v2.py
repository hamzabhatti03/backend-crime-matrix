...
def generate_username(designation, station, district):
    ...
    station_clean = clean_text(
        station.lower().replace(" ", ".").replace(district.lower(), "").replace(".pur.", "pur.").replace(".pur", "pur"))
...
def format_last_name(station, district, designation):
    ...
    station_clean = clean_text(station.replace("PS.", "").replace("PS", ""))
...
for _, row in df.iterrows():
    if row["Designation"].strip().lower() not in allowed_designations:
        continue
