'''
Code processing file to incorporate PUNJAB TODAY PROCESSING LOGIC USING CENTRALIZED DB
Latest file before this file for processing is db_processing_script_v4.py
'''
...
def process_date(db_connection, start_timestamp, end_timestamp):
    try:
        ...
        queries = {
            'total_calls': """
                    SELECT district_id, HOUR(FROM_UNIXTIME(time_id)) AS hour, COUNT(*) AS count
...
""",
            'conference_calls': """
                    SELECT l.district_id,HOUR(FROM_UNIXTIME(o.call_time_unix)) AS hour,COUNT(*) as count
                    FROM `outgoing_calls_logs_preprocessed` o
                    JOIN `15_preprocessed` l ON o.lead_id = l.lead_id
                    WHERE l.district_id IS NOT NULL AND o.tx_type='ATT' AND o.call_time_unix BETWEEN %s AND %s
                    GROUP BY hour,l.district_id
                """
        }
        ...
        for key, query in queries.items():
            ...
            if key == 'counts':
                ...
                for district_id, hour, siraiki_count, punjabi_count, potohari_count, vwps_count, traffic_count, english_count, transfered_count, call_backs_count, vccs_count, vcm_count, generated_cases_count in cursor.fetchall():
                    if isinstance(district_id, bytes):
                        district_id = district_id.decode('utf-8')

                    if (district_id, hour) not in results:
                        results[(district_id, hour)] = {}
                    results[(district_id, hour)].update({
                        ...
                        'transfered': int(transfered_count) if isinstance(transfered_count, Decimal) else transfered_count,
                        'call_backs': int(call_backs_count) if isinstance(call_backs_count, Decimal) else call_backs_count,
                        ...
                        'generated_cases': int(generated_cases_count) if isinstance(generated_cases_count, Decimal) else generated_cases_count
                    })

            elif key == 'total_calls':
                for district_id, hour, count in cursor.fetchall():
                    if isinstance(district_id, bytes):
                        district_id = district_id.decode('utf-8')
                    if (district_id, hour) not in results:
                        results[(district_id, hour)] = {}
                    results[(district_id, hour)]['total_calls'] = count
                ...
                for district_id, hour, count in cursor.fetchall():
                    ...
                    if (district_id, hour) not in results:
                        results[(district_id, hour)] = {}
                    results[(district_id, hour)]['conference_calls'] = count


        queries_without_district = {
            'app_alerts': """
                    SELECT HOUR(FROM_UNIXTIME(time_id)) AS hour, COUNT(*) AS count  
        ...
        for key, query_without_district in queries_without_district.items():
            ...
            for hour, count in cursor.fetchall():
                ...
                results[(configs.DEFAULT_DISTRICT_ID, hour)][key] = count
...
def insert_results(db_connection, results, date):
    try:
        ...
        INSERT OR REPLACE INTO processed_data (
            date, hour, district_id, total_calls,generated_cases, siraiki, punjabi, potohari, vwps, traffic, english, app_alerts, 
            transfered, call_backs, avg_response_time, estimated_response_time, 
            video_calls, conference_calls, vccs, vcm
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?)
        """
        ...
        for (district_id, hour), result in results.items():
            row = {
                'date': date,
                'hour': f"{hour}",
            ...
            cursor.execute(insert_query, (
                row['date'], row['hour'], row['district_id'], row['total_calls'], row['generated_cases'],row['siraiki'],
                row['punjabi'],row['potohari'], row['vwps'], row['traffic'], row['english'], row['app_alerts'], row['transfered'],
                row['call_backs'], row['avg_response_time'], row['estimated_response_time'],row['video_calls'],
                row['conference_calls'], row['vccs'], row['vcm']
            ))
...
def response_time(primary_conn, processed_conn, start_timestamp, end_timestamp):
    try:
        ...
        GROUP BY 
            ...
            l.accepted_time, l.level1_case_nature, l.level2_case_nature, l.level3_case_nature;
        """
        ...
        for processed_row in processed_results:
            processed_cursor.execute("""
                        INSERT OR REPLACE INTO response_time (
                            ...
                            region_category, level1_case_nature, level2_case_nature, level3_case_nature
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)
                    """, processed_row)
        processed_conn.commit()
...
def insert_punjab_today(db_connection, results, date):
    try:
        ...
        for (district_id, police_station), result in results.items():
            ...
            cursor.execute(insert_query, (
                row['date'], row['police_station'], row['district_id'],row['murder'],row['minorities'],row['hurt'],row['burglary'], row['aerial_firing'], row['rape'], row['kidnapping'],
                row['robbery_snatching'],row['dacoity_with_murder'],row['dacoity'], row['motorcycle_theft'], row['car_theft'],
...
def process_fir_cases(processed_conn, date):
    try:
        ...
        for category, count in results:
            if category in fir_data:
                fir_data[category] = count

        insert_query = """
            INSERT OR REPLACE INTO fir_cases (
                date, murder, dacoity, aerial_firing, rape, minorities, hurt, burglary,
                kidnapping, child_abuse, robbery_snatching, motorcycle_theft, dacoity_with_murder,
                car_theft, theft, terrorist_act, other_person, other_property
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        ...
        cursor.execute(insert_query, (
            ...
            fir_data['murder'],
            fir_data['dacoity'],
            fir_data['aerial_firing'],
            fir_data['rape'],
            fir_data['minorities'],
            fir_data['hurt'],
            fir_data['burglary'],
            fir_data['kidnapping'],
            fir_data['child_abuse'],
            fir_data['robbery_snatching'],
            fir_data['motorcycle_theft'],
            fir_data['dacoity_with_murder'],
            fir_data['car_theft'],
            fir_data['theft'],
            fir_data['terrorist_act'],
            fir_data['other_person'],
            fir_data['other_property']
        ))
...
def main(start_date, end_date, start):
    try:
        ...
        processed_conn = utils.get_db_connection(configs.PROCESSED_STATS_TEST_DB)
        processed_cursor = processed_conn.cursor()
...
if __name__ == '__main__':
    start_date = datetime.strptime('07-12-24', '%d-%m-%y')
    end_date = datetime.strptime('09-12-24', '%d-%m-%y')
